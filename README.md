...
Some systems may be not have all required functions built in. For full functionality make sure `watch` is installed (e.g. `brew install watch` on mac), and `column` (e.g. on linux might need to do `apt install util-linux` and/or `apt install bsdmainutils`).
